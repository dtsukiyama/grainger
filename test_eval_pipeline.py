...
def create_test_cases_from_dataframe(labels, predicted):
    """
    ...
    for idx in range(len(labels)):
        ...
        current_label = labels.loc[idx].label
        predicted_label = predicted.loc[idx].query_is_correct
        # standardize labels
        if current_label == "E":
            actual_output = "correct"
        else:
            actual_output = "incorrect"
...
def main():
    ...
    try:
        ...
        labels1 = pd.read_csv('data/label_df1.csv')
        evaluated1 = pd.read_csv('data/evaluated_labels_df1.csv')
        test_cases1 = create_test_cases_from_dataframe(labels1, evaluated1)
        logging.info(f"Created {len(test_cases1)} test cases from dataset 1")

        labels2 = pd.read_csv('data/label_df2.csv')
        evaluated2 = pd.read_csv('data/evaluated_labels_df2.csv')
        test_cases2 = create_test_cases_from_dataframe(labels2, evaluated2)
        logging.info(f"Created {len(test_cases2)} test cases from dataset 2")

        labels3 = pd.read_csv('data/label_df3.csv')
        evaluated3 = pd.read_csv('data/evaluated_labels_df3.csv')
        test_cases3 = create_test_cases_from_dataframe(labels3, evaluated3)
...
if __name__ == "__main__":
    ...
    logging.info("deepeval test run test_eval_pipeline.py -n 4")